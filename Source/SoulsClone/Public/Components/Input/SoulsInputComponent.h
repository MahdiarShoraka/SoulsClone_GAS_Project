...
template <class UserObject, typename CallbackFunc>
void USoulsInputComponent::BindAbilityInputAction(const UDataAsset_InputConfig* InInputConfig,
	UserObject* ContextObject, CallbackFunc InputPressedFunc, CallbackFunc InputReleasedFunc)
{
	checkf(InInputConfig, TEXT("Input config data asset is null, cannot proceed with binding"));
	for (const FSoulsInputActionConfig& AbilityInputActionConfig : InInputConfig->AbilityInputActions)
	{
